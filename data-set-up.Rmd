...
knitr::opts_chunk$set(echo = F, out.width = "100%", dpi = 330, fig.align = "center", warning = F, message = F, comment = F)
library(dplyr)
library(magrittr)
library(purrr)
library(formattable)
...
req_data <- read.csv("./data/rdi_data.csv", check.names=FALSE) %>% 
  pivot_longer(cols = 2:last_col(),
               names_to = "name",
               values_to = "amount") %>% 
  filter(`Life Stage Group` == "Males 19–30 y")
...
scaled_nutrient_content <- full_join(nutrient_content, portion_filtered, by = join_by(fdc_id)) %>% 
  ...
  mutate("amount" = amount.x * portion_amount,
         "food" = description,
         "name" = name,
         "unit" = unit,
         .keep = "none")%>% 
...
joined_nut_content <- full_join(scaled_nutrient_content, req_data, by = join_by(name)) %>% 
  drop_na() %>% 
  select(!`Life Stage Group`) %>% 
  mutate("Dietary Intake" = amount.x,
         "Recommended Intake" = amount.y,
         "Unit of Measure" = unit,
         "Percent Intake" = amount.x/amount.y,
         .keep = 'none') %>% 
  arrange(desc(`Percent Intake`))

selected_data <- tibble("foods" = food_selected,
                         "portions" = portion_amount,
                         "portion size" = portion_selected)
...
```{r}
as.datatable(formattable(joined_nut_content,
            list(`Percent Intake` = percent,
                 `Percent Intake` = 
                   formatter("div",
                             style = ~style(color = if_else(`Percent Intake` > 1.5 | `Percent Intake` < .80, "black", "green"))))))
```
...
```{r}
## Insert food breakdown plot


ggplot(joined_nut_content, aes(fct_reorder(name,`Dietary Intake`), `Dietary Intake`, fill = `Unit of Measure`))+
  geom_col()+
  geom_point(aes(name, `Recommended Intake`), color = "black",shape = "cross", size = 2)+
  facet_wrap(vars(`Unit of Measure`), scales = "free")+
 ...
 ggplot(joined_nut_content, aes(fct_reorder(name,`Percent Intake`), `Percent Intake`))+
  geom_col(fill = "ghostwhite", color ='black')+
  labs(title = "Total dietary intake",
       x = "Nutrient",
       y = "Amount")+
